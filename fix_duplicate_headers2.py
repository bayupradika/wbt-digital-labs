...
def fix_header(filepath):
    try:
        ...
        if not global_header:
            # print(f"No global_header in {filepath}")
            return False
            
        global_right = global_header.find('div', class_='wbt-global-header-right')
        if not global_right:
            print(f"No global_right in {filepath}")
            return False
            
        old_header = soup.find('div', class_='nav-bar')
        if not old_header:
            old_header = soup.find('div', class_='header-nav')
        if not old_header:
            old_header = soup.find('div', class_='header-container')
        if not old_header:
            old_header = soup.find('header', class_='header')
        if not old_header:
            old_header = soup.find('div', class_='top-header')
            
        if not old_header:
            print(f"No old_header in {filepath}")
            return False
